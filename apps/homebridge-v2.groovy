...
def deviceSelectPage() {
    return dynamicPage(name: 'deviceSelectPage', title: sBLANK, install: false, uninstall: false) {
        ...
        section(sectHead('Thermostats:')) {
            ...
            input 'tstatCoolList', 'capability.thermostat', title: inTS1("Cool Only Thermostats: (${tstatCoolList ? tstatCoolList.size() : iZ} Selected)", 'thermostat'), description: inputFooter(sTTS, sCLRGRY, true), multiple: true, submitOnChange: true, required: false
...
Map<String,Integer> deviceCapabilityList(device) {
    ...
    if (isDeviceInInput('tstatCoolList', devid)) { capItems['Thermostat'] = i1; capItems['ThermostatOperatingState'] = i1; capItems.remove('ThermostatHeatingSetpoint') }
    ...
    if (!remKeys) remKeys = []
    Boolean sdl = bgtSetting('showDebugLogs')
    remKeys.each { String k->
        String capName = k.replaceAll('remove', sBLANK)
...
Map<String,Integer> deviceCommandList(device) {
    String devid= (String)device?.getId()
    if (!device || !devid) { return [:] }
    Map<String,Integer> cmds = device.supportedCommands?.findAll { !ignoreCommand((String)it.name) }?.collectEntries { c-> [ ((String)c.name) : i1 ] }
...
private static Boolean ignoreCommand(String cmd) {
    return cmd && (cmd in ignoreListFLD.commands)
}
...
private Boolean ignoreAttribute(device, String iattr, Boolean inclIgnoreFld=true) {
    ...
    if (inclIgnoreFld && (attr in ignoreListFLD.evt_attributes.collect { it.toLowerCase() })) { return true }
...
Map<String,Object> deviceAttributeList(device) {
    ...
    Map<String,Object> atts = ((List)device.getSupportedAttributes())?.findAll { !( (String)it.name in ignoreListFLD.attributes || (String)it.name in ignoreListFLD.evt_attributes ) }?.collectEntries { attribute->
